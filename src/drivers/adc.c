...
static volatile uint16_t
    adc_results[4]; // Array to store results of ADC channels A0, A1, A2, A3
static volatile uint16_t
    adc_cache[4];             // Cache for storing last ADC conversion results
...
static uint8_t
    adc_channel_count; // Total number of ADC channels (based on available pins)
...
void adc_init(void) {
  ...
  adc_channel_count = adc_pin_count;      // Set total number of channels to the
  ...
  ADC12CTL0 = ADC12ON + ADC12MSC +
              ADC12SHT0_8; // Turn on ADC12, set extended sampling time
  ADC12CTL1 = ADC12SHP + ADC12CONSEQ_3 +
              ADC12SSEL1;   // Use sampling timer, repeated sequence mode
